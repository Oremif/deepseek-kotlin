 ...
 *
 * The SDK deserialises this payload and wraps it inside a [DeepSeekException] together with the
 * HTTP status code and headers. Use [DeepSeekException.error] to access it from a caught exception.
 *
 * @property error The nested error object describing what went wrong
 */
@Serializable
public class DeepSeekError(public val error: Error) {
    /**
     * Details about a DeepSeek API error.
     *
     * All fields are nullable because the API does not guarantee their presence on every failure
     * mode.
     *
 ...
 *
 * Thrown by every endpoint extension function (chat, FIM, models, userBalance, and their streaming
 * counterparts) when the API returns a non-2xx status. Use [statusCode] and the subclass type to
 * distinguish failure modes, [error] to inspect the structured error payload, and [headers] for
 * rate-limit or retry hints.
 *
 * Example:
 * ```kotlin
 * try {
...
public sealed class DeepSeekException(
     ...
     *
     * Inspect [error] for details on which field is invalid and adjust the request accordingly
     * before retrying.
     */
     ...
     *
     * Verify the token passed to [org.oremif.deepseek.client.DeepSeekClient] or create a new key at
     ...
     *
     * Top up the account at
     * [https://platform.deepseek.com/top_up](https://platform.deepseek.com/top_up) or check the
     * current balance with [org.oremif.deepseek.api.userBalance].
     */
    ...
    /**
     * Thrown for HTTP 422 — the request is syntactically valid but semantically invalid (for
     * example, a parameter is outside its allowed range).
     */
     ...
     *
     * Retry after a brief wait. The Ktor retry plugin already retries this status automatically up
    ...
    public companion object {
        ...
        public fun from(
            ...
            error: DeepSeekError?,
            message: String,
        ): DeepSeekException = create(statusCode, headers, error, message)
        ...
        public fun from(
            ...
            headers: DeepSeekHeaders,
            error: DeepSeekError?,
        ): DeepSeekException = create(statusCode, headers, error, defaultMessageFor(statusCode))
