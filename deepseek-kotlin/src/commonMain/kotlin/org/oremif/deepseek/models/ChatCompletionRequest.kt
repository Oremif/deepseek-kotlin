 ...
 * @property responseFormat An object specifying the format that the model must output. Setting to {
 *   "type": "json_object" } enables JSON Output, which guarantees the message the model generates
 *   is valid JSON.
 ...
 *   server-sent events (SSE) as they become available, with the stream terminated by a `data:
 *   [DONE]`
 ...
 *   mode is on. See [ReasoningEffort].
 *
 * **Possible values: {`low`, `high`, `max`}.**
 *
 * **Default value: `high`.**
 *
 * @property userId A custom identifier of the end user behind the request, used for content safety
...
public class ChatCompletionRequest(
     ...
     *
     * Used by `DeepSeekClient.chatCompletion { ... }`; compose [messages] and [params] inside the
     * block.
     */
    public class Builder {
        ...
        internal fun build(): ChatCompletionRequest = params.createRequest(messages)
    }

    /**
     * Builder for streaming chat completion requests.
     *
     * Used by `DeepSeekClientStream.chatCompletion { ... }`; compose [messages] and [params] inside
     * the block. The resulting request has `stream = true`.
     */
     ...
     *
     * Appends messages to an internal list in call order. Each helper adds the matching concrete
     * [ChatMessage] subtype.
     *
     * Example:
     * ```kotlin
    ...
    public class UserContentBuilder {
        ...
        internal fun build(): List<ContentPart> {
            require(parts.isNotEmpty()) {
                "a user message built with the parts DSL must have at least one part"
            }
            return parts.toList()
        }
    }
