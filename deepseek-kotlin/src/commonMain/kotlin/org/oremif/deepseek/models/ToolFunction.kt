 ...
 *
 * Has two concrete forms: [FunctionRequest] — the signature the model may call, sent in the request
 * — and [FunctionResponse] — the call the model actually emitted, received in the response. The
 * [ToolFunctionSerializer] picks the right form at deserialization based on the presence of
 ...
 * @property description Natural-language description of what the function does; helps the model
 *   decide when to call it.
 * @property parameters JSON-Schema description of the arguments the function accepts.
 * @property strict Whether the API constrains the generated arguments to [parameters] instead of
 *   merely prompting for them. Beta feature; left out of the request when `null`, which the API
 *   reads as `false`.
 */
@Serializable
 ...
 *   valid JSON, nor to match the schema declared in the corresponding [FunctionRequest.parameters];
 *   [argumentsAsJsonOrNull] parses it defensively.
 */
@Serializable
public class FunctionResponse(
    ...
    override fun toString(): String = "FunctionResponse(name='$name', arguments=$arguments)"
}

/**
 * Parses [FunctionResponse.arguments] into a [JsonObject], or returns `null` when the model
 * produced no arguments, malformed JSON, or a JSON value that is not an object.
 *
 * Example:
 * ```kotlin
 ...
 */
public fun FunctionResponse.argumentsAsJsonOrNull(json: Json = Json): JsonObject? = arguments?.let {
    runCatching { json.parseToJsonElement(it) as? JsonObject }.getOrNull()
}

internal object ToolFunctionSerializer :
    JsonContentPolymorphicSerializer<ToolFunction>(ToolFunction::class) {
    override fun selectDeserializer(element: JsonElement): DeserializationStrategy<ToolFunction> {
        val obj =
            element as? JsonObject
                ?: throw SerializationException(
                    "Expected JsonObject for ToolFunction, got: ${element::class.simpleName}"
                )
        return when {
