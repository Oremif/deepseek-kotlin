...
class DeepSeekExceptionTests {
    ...
    fun `DeepSeekError deserializes missing param as null`() {
        val json =
            """{"error":{"message":"Something failed","type":"server_error","code":"internal_error"}}"""
        val error = Json.decodeFromString(DeepSeekError.serializer(), json)
        error.error.param.shouldBeNull()
    }
    ...
    fun `OverloadServerException exposes 503 as statusCode`() {
        val ex =
            DeepSeekException.OverloadServerException(DeepSeekHeaders.Empty, null, "overloaded")
    ...
    fun `both from overloads return OverloadServerException for 503`() {
        ...
        val fromLong =
            DeepSeekException.from(503, DeepSeekHeaders.Empty, null, "Service Unavailable")
    ...
    fun `message has no leading newline when error is null`() {
        val ex =
            DeepSeekException.from(401, DeepSeekHeaders.Empty, null, "Please check your API key.")
    ...
    fun `message is empty when both error and fallback message are absent`() {
        val ex =
            DeepSeekException.UnexpectedStatusCodeException(418, DeepSeekHeaders.Empty, null, null)
        ex.message shouldBe ""
    }
    ...
    fun `message contains only error message when fallback message is null`() {
        ...
        val ex =
            DeepSeekException.UnexpectedStatusCodeException(418, DeepSeekHeaders.Empty, error, null)
    ...
    fun `message joins error and fallback with newline when both present`() {
        val error = DeepSeekError(DeepSeekError.Error(message = "Invalid API key"))
        val ex =
            DeepSeekException.from(401, DeepSeekHeaders.Empty, error, "Please check your API key.")
