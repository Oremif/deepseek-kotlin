...
class UserMessageContentTests {
    ...
    fun `a plain-text user message still serializes content as a string`() {
        TestJson.encodeToString<ChatMessage>(UserMessage("Hi")) shouldBe
    ...
    fun `a file part serializes flat next to its type discriminator`() {
        ...
        val inline =
            UserMessage(
                listOf(FilePart(fileData = "data:image/png;base64,AAAA", filename = "cat.png"))
            )
    ...
    fun `an image url part omits an unset detail`() {
        TestJson.encodeToString<ChatMessage>(
            UserMessage(listOf(ImageUrlPart("https://example.com/a.jpg")))
        ) shouldBe
    ...
    fun `a multimodal user message survives a round trip`() {
        val message =
            UserMessage(
                listOf(
                    TextPart("look"),
                    ImageUrlPart("https://example.com/a.jpg", ImageDetail.HIGH),
                ),
                name = "alice",
            )

        val decoded =
            TestJson.decodeFromString<ChatMessage>(TestJson.encodeToString<ChatMessage>(message))
    ...
    fun `the parts DSL sends parts in the order they were appended`() {
        ...
        parts.map { it::class } shouldBe
            listOf(ImageUrlPart::class, TextPart::class, FilePart::class)
